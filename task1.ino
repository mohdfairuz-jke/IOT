...
//6.2 Function - update_suhu() field1
...
//6.2 Function - update_suhu() field2
void update_field2(float dd) {
  Serial.println("\nStarting connection to server...");
  client.setInsecure();
  if (!client.connect(host, port))
    Serial.println("Connection failed!");
    ...
    //Prepare HTTP header
    String header = "";
    header = "GET /update?api_key=" + apikey + "&field2=" + (String(dd)) + " HTTP/1.1\n";
    header += "Host: api.thingspeak.com\n";
    header += "Connection: close\n\n";
    Serial.print(header);
    // Make a HTTP request:
...
void loop() {
  ...
  // Convert Lux to Temperature
  temperature = 20.0 + (luxValue * 0.02);

  //Read state button sw1
  byte button1State = digitalRead(sw1Pin34);
  byte button2State = digitalRead(sw2Pin32);

  // S1 → Upload Temperature (Field 1)
  ...
  // S2 → Upload Lux Value (Field 2)
  if (button2State == HIGH) {
    delay(500);   // debounce
    Serial.print("Light Intensity (Lux): ");
    Serial.println(luxValue);
    update_field2(luxValue);
    Serial.println("Wait 15 sec for next update");
    delay(15000);   // ThingSpeak limit
  }
}
